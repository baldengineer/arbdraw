...
function nextLower125(value){if(!Number.isFinite(value)||value<=0)return value;const exponent=Math.floor(Math.log10(value)),power=10**exponent,normalized=value/power,epsilon=1e-12;for(const step of [5,2,1])if(step<normalized-epsilon)return step*power;return 5*power/10}
function samplesForOneCycle(rateMSa,frequencyHz){return Math.max(2,Math.ceil(rateMSa*1e6/frequencyHz))}
