...
function syncInputs(){state.high=inputVoltage('highInput');state.low=inputVoltage('lowInput');if(state.high<state.low)[state.high,state.low]=[state.low,state.high];state.frequency=Math.max(.000001,+$('frequencyInput').value);state.cycles=state.frequency*state.duration/1000;state.phase=+$('phaseInput').value;state.duty=+$('dutyInput').value;$('highInput').value=displayVoltage('highInput',state.high);$('lowInput').value=displayVoltage('lowInput',state.low);$('amplitudeInput').value=displayAmplitude(state.high-state.low);$('offsetInput').value=displayVoltage('offsetInput',(state.high+state.low)/2);$('dutyValue').textContent=state.duty+'%';renderFrequency();renderTiming();}
...
function propertiesDiffer(){const values=[inputVoltage('highInput'),inputVoltage('lowInput'),+$('frequencyInput').value,+$('phaseInput').value,+$('dutyInput').value],current=[state.high,state.low,state.frequency,state.phase,state.duty];return values.some((value,index)=>!Number.isFinite(value)||Math.abs(value-current[index])>Math.max(1,Math.abs(current[index]))*1e-10)}
...
function setPropertyInputDefault(input){const key=propertyDefaultMap[input.id];if(!key)return;input.value=input.id==='periodInput'?Number((1e6/DEFAULT_VALUES.frequencyHz).toPrecision(10)):input.id==='amplitudeInput'?displayAmplitude(DEFAULT_VALUES.amplitudeVpp):voltageUnitScales[input.id]?displayVoltage(input.id,DEFAULT_VALUES[key]):DEFAULT_VALUES[key];input.dispatchEvent(new Event('input',{bubbles:true}));applyProperties()}
